...
def seeded(db):
    ...
    def make(order_number, customer, price, *, final=False, returnable=True,
             delivered_days=5, status="delivered", sku="SKU-1", category="electronics"):
        ...
        db.flush()
        db.add(OrderItem(
            order_id=order.id, product_name="Widget", sku=sku, category=category,
            quantity=1, unit_price=price, is_final_sale=final, is_returnable=returnable,
        ))
        return order
